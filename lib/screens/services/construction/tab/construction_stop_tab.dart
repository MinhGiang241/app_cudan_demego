...
import '../construction_stop_screen.dart';
import '../prv/construction_list_prv.dart';

class ConstructionStopTab extends StatefulWidget {
  const ConstructionStopTab({
    super.key,
    required this.getList,
    required this.list,
  });

  final List<ConstructionTemporarilyStopped> list;
  final Function(BuildContext) getList;

  @override
  State<ConstructionStopTab> createState() => _ConstructionStopTabState();
}

class _ConstructionStopTabState extends State<ConstructionStopTab> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        List<ConstructionTemporarilyStopped> waitOwnerLetter = [];
        List<ConstructionTemporarilyStopped> waitTechLetter = [];
        List<ConstructionTemporarilyStopped> waitManagerLetter = [];
        List<ConstructionTemporarilyStopped> approvedLetter = [];
        List<ConstructionTemporarilyStopped> cancelLetter = [];
          ...
          return SafeArea(
            child: SmartRefresher(
              ...
              child: PrimaryEmptyWidget(
                emptyText: S.of(context).no_stop_letter,
              ...
              children: [
                ...
                ...list.map((e) {
                  var listContent = [
                    ...
                    InfoContentView(
                      title: "${S.of(context).start_date_stop}: ",
                      content: e.temporarily_stopped != null &&
                              e.temporarily_stopped!.length >= 1 &&
                              e.temporarily_stopped?[0]?.length >= 1
                          ? Utils.dateFormat(
                              e.temporarily_stopped?[0]?[0] ?? '',
                              1,
                            )
                          : null,
                      contentStyle: txtBold(14, grayScaleColorBase),
                    ...
                    InfoContentView(
                      title: '${S.of(context).end_date_stop}: ',
                      content: e.temporarily_stopped != null &&
                              e.temporarily_stopped!.length >= 1 &&
                              e.temporarily_stopped?[0]?.length >= 2
                          ? Utils.dateFormat(
                              e.temporarily_stopped?[0]?[1] ?? '',
                              1,
                            )
                          : null,
                      contentStyle: txtBold(14, grayScaleColorBase),
                                ...
                                PrimaryButton(
                                  onTap: () {
                                    context
                                        ...
                                        .cancelConstructionStop(context, e);
                                  },
